...
// const std::string_view prefix{"./simul_results/MCH_100events_fmu"}; // prefix
// of simulation directory
/*
const uint32_t nOrbitsPerTF = 128;
=======
const std::string_view prefix{"./simul_results/MCH_10events_fmu"}; // prefix of
simulation directory const uint32_t nOrbitsPerTF = 128;
...
void test_Alignement(std::string prefix, std::string mchFileName,
  ...
  // const auto grp =
  // parameters::GRPObject::loadFrom(base::NameConf::getGRPFileName(prefix));
  // const auto grp = parameters::GRPObject::loadFrom();
  ...
  */
  ...
  // Some configuration
  test_align->SetAllowedVariation(0, 1.5);
  test_align->SetAllowedVariation(1, 1.5);
  ...
  while (mchReader->Next() && muonReader->Next()) {
    ...
    for (const auto &mchROF : *mchROFs) {
      ...
      for (int iMCHTrack = mchROF.getFirstIdx();
        ...
        //   // Data format converision: O2 convention to MCH internal
        //   convention:
        //   // TrackMCH -> Track
        ...
        cout << nb_clusters << " clusters attached for track " << id_track
             << endl;

        // Data format conversion

        // mch::Track* alignTrack = MCHFormatConvert(mchTrack,
        // *mchClusters);
        ...
        */
        ...
        // cout << "old param => x slope: " << Param0[1] << " y slope: " <<
        // Param0[3]
        //      << endl;
        for (int id_cluster = id_cluster_first;
          ...
          */
          ...
          // cout << "Cluster: " << id_cluster << " at Z = " << Z_cluster <<
          // endl; cout << "Cluster ID  "
          //  << "DET: " << DEId_cluster << " Chamber: " << CId_cluster << "
          //  Index: " << ind_cluster << endl;

          // cout <<
          // "-----------------------------------------------------------------------"
          // <<endl; Perform extrapolation to get all track parameters
          /*
            will there be some extra bias added while processing
            extrapolation ?
          */
          if (mch::TrackExtrap::extrapToZ(extrapParam, Z_cluster)) {
            LOG(debug) << Form("Extrapolation succes to cluster: %i",
                               id_cluster);
          } else {
            LOG(fatal) << Form("Extrapolation failed to cluster: %i",
                               id_cluster);
          }

          // cout << "new param => x slope: " <<
          // extrapParam.getNonBendingSlope()
          // << " y slope: " << extrapParam.getBendingSlope() << endl; cout
          // <<
          // "-----------------------------------------------------------------------"
          // << endl;
          ...
          for(;itParam!=convertedTrack.end();++itParam){
            ...
            LOG(info) << Form("cluster ID: %i    chamber ID: %i",
          itCluster->getDEId(), itCluster->getChamberId());
        ...
        // cout << convertedTrack.getNClusters()
        //      << " clusters' param are converted into MCH track format."
        //      << endl;
        // cout <<
        // "----------------------------------------------------------------"
        //         "-------"
        ...
        // cout << alignTrack->getNClusters() <<" clusters for current
        // track"<< endl; auto param_ptr = convertedTrack->begin(); const
        // mch::Cluster* cluster = param_ptr->getClusterPtr(); cout <<
        // "first cluster detector ID: " << cluster->getDEId() <<endl; no
        // problem with alignTrack
  ...
  /*
    track records are stored in test_algin.fMillepede.fTreeData (with
    branch fRecord and same indexing with tracks) and can be read using
    ReadRecordData(Long_t recID)
  */
...
mch::Track *MCHFormatConvert(mch::TrackMCH &track,
  ...
  cout << "Current track's cluster starts at index: " << id_cluster_first
       << " stops at: " << id_cluster_last << endl;
  cout << "old param => x slope: " << Param0[1] << " y slope: " << Param0[3]
       << endl;
  for (int id_cluster = id_cluster_first; id_cluster < id_cluster_last + 1;
       ++id_cluster) {
    ...
    // cout << "new param => x slope: " << extrapParam->getNonBendingSlope()
    //      << " y slope: " << extrapParam->getBendingSlope() << endl;
    // cout << "------------------------------------------------------------"
    //         "------"
    //         "-----"
    //      << endl;
  ...
  cout << convertedTrack->getNClusters()
       << " clusters' param are converted into MCH track format." << endl;
  cout << "--------------------------------------------------------------"
          "------"
          "---"
       << endl;
...
2. Problem while creating MCH type cluster: same namespace used for common
data format
